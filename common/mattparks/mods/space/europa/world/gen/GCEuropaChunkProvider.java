		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < GCEuropaChunkProvider.CHUNK_SIZE_X; x++)
				{
					...
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * GCEuropaChunkProvider.CHUNK_SIZE_X + x, cz * GCEuropaChunkProvider.CHUNK_SIZE_Z + z) / GCEuropaChunkProvider.CRATER_PROB)
						{
