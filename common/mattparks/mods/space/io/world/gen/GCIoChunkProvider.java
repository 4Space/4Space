...
{
	final short topBlockID = (short) IoBlocks.IoSlab.blockID;
	final byte topBlockMeta = 5;
	final short fillBlockID = (short) IoBlocks.IoFull.blockID;
...
//	final short lowerBlockID = (short) Block.waterMoving.blockID;
	...
	public GCIoBiomeDecorator biomedecoratorplanet = new GCIoBiomeDecorator();
	...
	public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < GCIoChunkProvider.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < GCIoChunkProvider.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < GCIoChunkProvider.CHUNK_SIZE_Y; y++)
				{
					if (y < GCIoChunkProvider.MID_HEIGHT + yDev)
					{
