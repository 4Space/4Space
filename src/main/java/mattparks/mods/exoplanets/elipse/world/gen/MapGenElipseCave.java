...
public class MapGenElipseCave extends MapGenBaseMeta {
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										...
										if (yfactor > -0.7D && xfactorSq + yfactorSq + zfactorSq < 1.0D) {
											if (blockIdArray[coords] == ElipseBlocks.elipseBasicBlock) {
												if (metaArray[coords] == 1 || metaArray[coords] == 2) {
													blockIdArray[coords] = Blocks.air;
												} else if (metaArray[coords] == 0 && random.nextInt(MapGenElipseCave.BREAK_THROUGH_CHANCE) == 0) {
													blockIdArray[coords] = Blocks.air;
												}
											}
										}
