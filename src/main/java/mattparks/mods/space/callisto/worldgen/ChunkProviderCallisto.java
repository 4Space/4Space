...
import mattparks.mods.space.core.dungeon.RoomBossSpace;
import mattparks.mods.space.core.dungeon.RoomChestsSpace;
import mattparks.mods.space.core.dungeon.RoomEmptySpace;
import mattparks.mods.space.core.dungeon.RoomSpawnerSpace;
import mattparks.mods.space.core.dungeon.RoomTreasureSpace;
...
public class ChunkProviderCallisto extends ChunkProviderGenerate
{
	final Block topBlockID = CallistoBlocks.callistoBasicBlock;
	final byte topBlockMeta = 0;
	final Block fillBlockID = CallistoBlocks.callistoBasicBlock;
	final byte fillBlockMeta = 1;
	final Block lowerBlockID = CallistoBlocks.callistoBasicBlock;
	...
	public BiomeDecoratorCallisto biomedecoratorplanet = new BiomeDecoratorCallisto();

	private final World worldObj;

	private final MapGenDungeon dungeonGenerator = new MapGenDungeon(ErisBlocks.erisBasicBlock, 10, 8, 16, 3);

	{
		this.dungeonGenerator.otherRooms.add(new RoomEmptySpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomChestsSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomSpawnerSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		...
		this.dungeonGenerator.otherRooms.add(new RoomChestsSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.otherRooms.add(new RoomChestsSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.bossRooms.add(new RoomBossSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
		this.dungeonGenerator.treasureRooms.add(new RoomTreasureSpace(null, 0, 0, 0, ForgeDirection.UNKNOWN));
	}

	private BiomeGenBase[] biomesForGeneration = { BiomeGenBaseCallisto.callisto };
	...
	public ChunkProviderCallisto(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < ChunkProviderCallisto.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < ChunkProviderCallisto.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < ChunkProviderCallisto.CHUNK_SIZE_Y; y++)
				{
					if (y < ChunkProviderCallisto.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < ChunkProviderCallisto.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < ChunkProviderCallisto.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderCallisto.CHUNK_SIZE_X + x, cz * ChunkProviderCallisto.CHUNK_SIZE_Z + z) / ChunkProviderCallisto.CRATER_PROB)
						{
