...
import mattparks.mods.space.mercury.world.gen.dungeon.RoomBossMercury;
import mattparks.mods.space.mercury.world.gen.dungeon.RoomChestsMercury;
import mattparks.mods.space.mercury.world.gen.dungeon.RoomEmptyMercury;
import mattparks.mods.space.mercury.world.gen.dungeon.RoomSpawnerMercury;
import mattparks.mods.space.mercury.world.gen.dungeon.RoomTreasureMercury;
...
public class ChunkProviderMercury extends ChunkProviderGenerate {
	Block topBlockID = MercuryBlocks.mercuryBasicBlock;
	byte topBlockMeta = 0;
	Block fillBlockID = MercuryBlocks.mercuryBasicBlock;
	byte fillBlockMeta = 1;
	Block lowerBlockID = MercuryBlocks.mercuryBasicBlock;
	...
	public BiomeDecoratorMercury biomedecoratorplanet = new BiomeDecoratorMercury();
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray) {
		...
		for (int x = 0; x < ChunkProviderMercury.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderMercury.CHUNK_SIZE_Z; z++) {
				double baseHeight = this.noiseGen1.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderMercury.TERRAIN_HEIGHT_MOD;
				double smallHillHeight = this.noiseGen2.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderMercury.SMALL_FEATURE_HEIGHT_MOD;
				...
				double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderMercury.MAIN_FEATURE_FILTER_MOD;
				double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderMercury.LARGE_FEATURE_FILTER_MOD;
				double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderMercury.SMALL_FEATURE_FILTER_MOD - 0.5;
				mountainHeight = this.lerp(smallHillHeight, mountainHeight * ChunkProviderMercury.MOUNTAIN_HEIGHT_MOD, this.fade(this.clamp(mountainHeight * 2, 0, 1)));
				valleyHeight = this.lerp(smallHillHeight, valleyHeight * ChunkProviderMercury.VALLEY_HEIGHT_MOD - ChunkProviderMercury.VALLEY_HEIGHT_MOD + 9, this.fade(this.clamp((valleyHeight + 2) * 4, 0, 1)));
	...
	public void replaceBlocksForBiome(int par1, int par2, Block[] arrayOfIDs, byte[] arrayOfMeta, BiomeGenBase[] par4ArrayOfBiomeGenBase) {
		...
		for (int var8 = 0; var8 < 16; ++var8) {
			for (int var9 = 0; var9 < 16; ++var9) {
				...
				for (int var16 = ChunkProviderMercury.CHUNK_SIZE_Y - 1; var16 >= 0; --var16) {
