...
public class ChunkProviderGanymede extends ChunkProviderGenerate
{
	final Block topBlockID = ErisBlocks.erisBasicBlock;
	final byte topBlockMeta = 0;
	final Block fillBlockID = ErisBlocks.erisBasicBlock;
	final byte fillBlockMeta = 1;
	final Block lowerBlockID = ErisBlocks.erisBasicBlock;
	...
	public BiomeDecoratorGanymede biomedecoratorplanet = new BiomeDecoratorGanymede();
	...
	private BiomeGenBase[] biomesForGeneration = { BiomeGenBaseGanymede.eris };
	...
	public ChunkProviderGanymede(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < ChunkProviderGanymede.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < ChunkProviderGanymede.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < ChunkProviderGanymede.CHUNK_SIZE_Y; y++)
				{
					if (y < ChunkProviderGanymede.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < ChunkProviderGanymede.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < ChunkProviderGanymede.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderGanymede.CHUNK_SIZE_X + x, cz * ChunkProviderGanymede.CHUNK_SIZE_Z + z) / ChunkProviderGanymede.CRATER_PROB)
						{
