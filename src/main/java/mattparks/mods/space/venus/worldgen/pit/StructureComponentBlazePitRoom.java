		...
		for (int y = 0; y < this.height; y++)
		{
			for (int x = 0; x < 7; x++)
			{
				for (int z = 0; z < 7; z++)
				{
					...
					{
						this.placeBlockAtCurrentPosition(world, VenusBlocks.venusBasicBlock, 13, x, y, z, this.getBoundingBox());
					}
					else
					{
						...
						}
						{
							for (int i = -2; i < 2; i++)
							{
								for (int j = -2; j < 2; j++)
								{
									...
									{
										this.placeBlockAtCurrentPosition(world, VenusBlocks.venusBasicBlock, 13, x + i, y, z + j, this.getBoundingBox());
									}
