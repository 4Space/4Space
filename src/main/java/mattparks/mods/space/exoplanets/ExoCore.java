...
public class ExoCore {
	...
	private static Planet planetElpis;
	private static Moon moonElpis_M1;
	private static Planet planetHestia;
	private static Moon moonHestia_M1;
	private static Planet planetHera;
	private static Moon moonHera_M1;
	private static Moon moonHera_M2;
	...
	public void init(FMLInitializationEvent event) {
		...
		ExoCore.planetElpis = (Planet) new Planet("elpis").setParentSolarSystem(ExoCore.gliese581g).setRingColorRGB(0.1F, 0.9F, 0.6F).setPhaseShift(0.3F).setRelativeSize(0.3519F).setRelativeDistanceFromCenter(new CelestialBody.ScalableDistance(0.45F, 0.45F)).setRelativeOrbitTime(1.86199F);
		ExoCore.planetElpis.setBodyIcon(new ResourceLocation(ExoCore.ASSET_PREFIX, "textures/gui/celestialbodies/elpis.png"));
		GalaxyRegistry.registerPlanet(ExoCore.planetElpis);
		
		ExoCore.moonElpis_M1 = (Moon) new Moon("elpis_m1").setParentPlanet(ExoCore.planetElpis).setRelativeSize(0.1656F).setRelativeDistanceFromCenter(new CelestialBody.ScalableDistance(17F, 17F)).setRelativeOrbitTime(1 / 0.001125F);
	...
	public void postInit(FMLPostInitializationEvent event) {
		//ExoCore.exoplanetsBlocksTab = new CreativeTabsSpace(CreativeTabs.getNextID(), "ExoplanetsBlocks", Item.getItemFromBlock(VenusBlocks.venusBasicBlock), 4);
		//ExoCore.exoplanetsItemsTab = new CreativeTabsSpace(CreativeTabs.getNextID(), "ExoplanetsItems", VenusItems.venusBasicItem, 2);
