...
import mattparks.mods.space.pluto.world.gen.dungeon.RoomBossPluto;
import mattparks.mods.space.pluto.world.gen.dungeon.RoomChestsPluto;
import mattparks.mods.space.pluto.world.gen.dungeon.RoomEmptyPluto;
import mattparks.mods.space.pluto.world.gen.dungeon.RoomSpawnerPluto;
import mattparks.mods.space.pluto.world.gen.dungeon.RoomTreasurePluto;
...
public class ChunkProviderPluto extends ChunkProviderGenerate {
	...
	public BiomeDecoratorPluto biomedecoratorplanet = new BiomeDecoratorPluto();

	private World worldObj;

	private MapGenVenusPluto caveGenerator = new MapGenVenusPluto();

	private MapGenDungeon dungeonGenerator = new MapGenDungeon(PlutoBlocks.plutoBasicBlock, 10, 8, 16, 6);
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray) {
		...
		for (int x = 0; x < ChunkProviderPluto.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderPluto.CHUNK_SIZE_Z; z++) {
				double baseHeight = this.noiseGen1.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderPluto.TERRAIN_HEIGHT_MOD;
				double smallHillHeight = this.noiseGen2.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderPluto.SMALL_FEATURE_HEIGHT_MOD;
				...
				double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderPluto.MAIN_FEATURE_FILTER_MOD;
				double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderPluto.LARGE_FEATURE_FILTER_MOD;
				double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderPluto.SMALL_FEATURE_FILTER_MOD - 0.5;
				mountainHeight = this.lerp(smallHillHeight, mountainHeight * ChunkProviderPluto.MOUNTAIN_HEIGHT_MOD, this.fade(this.clamp(mountainHeight * 2, 0, 1)));
				valleyHeight = this.lerp(smallHillHeight, valleyHeight * ChunkProviderPluto.VALLEY_HEIGHT_MOD - ChunkProviderPluto.VALLEY_HEIGHT_MOD + 9, this.fade(this.clamp((valleyHeight + 2) * 4, 0, 1)));
