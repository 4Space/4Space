...
public class ChunkProviderEris extends ChunkProviderGenerate
{
	...
	public BiomeDecoratorEris biomedecoratorplanet = new BiomeDecoratorEris();
	...
	private BiomeGenBase[] biomesForGeneration = { BiomeGenBaseEris.eris };
	...
	public ChunkProviderEris(World par1World, long par2, boolean par4)
	{
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
	{
		...
		for (int x = 0; x < ChunkProviderEris.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < ChunkProviderEris.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < ChunkProviderEris.CHUNK_SIZE_Y; y++)
				{
					if (y < ChunkProviderEris.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < ChunkProviderEris.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < ChunkProviderEris.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderEris.CHUNK_SIZE_X + x, cz * ChunkProviderEris.CHUNK_SIZE_Z + z) / ChunkProviderEris.CRATER_PROB)
						{
