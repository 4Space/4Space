...
public class RoomBossHole extends DungeonRoom
{
    public int sizeX;
    public int sizeY;
    public int sizeZ;
    ...
    public RoomBossHole(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
        ...
        for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
        {
            for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
            {
                for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
                {
                    ...
                    {
                        this.placeBlock(chunk, meta, i, j, k, cx, cz, VenusBlocks.evolvedBlazeEgg, 0);
                    }
                    else
                    {
    ...
    {
        return new RoomBossHole(dungeon, x, y, z, dir);
    }
